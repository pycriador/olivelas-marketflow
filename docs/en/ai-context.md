...
- The [`sdd/`](../../sdd/) files (`00-VISION.md` … `09-DESIGN_SYSTEM.md` and `10-FULL-SYSTEM.md`) are the product's source specification.
...
| **3** | [contracts/api.md](contracts/api.md) | Interfaces and contracts |
...
Source: `sdd/01-FOUNDATION.md`, `sdd/08-SECURITY.md`, `sdd/09-DESIGN_SYSTEM.md`.
...
- **Product AI:** AI is not the authoritative source; preferred flow is `image → AI → result + confidence → human review → save`. Never execute instructions coming from AI output. AI with quota, rate limit and auditing.
