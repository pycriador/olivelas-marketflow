...
import { Factory, Plus, Search, Edit, Globe, Phone, Mail } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map(mfg => (
          <Card key={mfg.id} className="hover:shadow-md transition-shadow">
            <CardHeader className="flex flex-row items-start justify-between pb-2">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <CardTitle className="text-base font-semibold">{mfg.name}</CardTitle>
                  {mfg.document && <p className="text-xs text-muted-foreground font-mono">CNPJ: {mfg.document}</p>}
                </div>
              </div>
              <Button variant="ghost" size="icon" onClick={() => handleOpenModal(mfg)}>
            ...
            <CardContent className="space-y-2 text-xs">
              {mfg.phone && (
                <p className="flex items-center text-muted-foreground">
                  <Phone className="mr-1.5 h-3.5 w-3.5" /> {mfg.phone}
                </p>
              )}
              {mfg.email && (
                <p className="flex items-center text-muted-foreground">
                  <Mail className="mr-1.5 h-3.5 w-3.5" /> {mfg.email}
                </p>
              )}
              {mfg.website && (
                <a href={mfg.website} target="_blank" rel="noreferrer" className="flex items-center text-primary hover:underline">
                  <Globe className="mr-1.5 h-3.5 w-3.5" /> {mfg.website}
                </a>
              )}
              <div className="pt-2">
